...
class ActividadController {
    ...
    list(req, res) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            const actividad = yield database_1.default.query('SELECT DISTINCT `id`, `nombre`, `descripcion`, `fecha`, `fechaEntrega`, `horaEntrega`, `noTrabajador`, `nrc`, `id_equipo`, `urlProfesor` FROM `actividad` WHERE nrc=? AND noTrabajador=? AND id_equipo IS NULL', [nrc, noTrabajador]);
            res.json(actividad);
        });
    }
    getOne(req, res) {
    ...
    deleteActividad(req, res) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            if (req.body.nrc > 0) {
                yield database_1.default.query('DELETE FROM actividad WHERE nrc=?', [req.body.nrc]);
                res.json({ message: 'The activitys was delated' });
            }
            else if (req.body.matricula > 0) {
                yield database_1.default.query('DELETE FROM actividad WHERE matricula=?', [req.body.matricula]);
                res.json({ message: 'The activitys was delated' });
            }
